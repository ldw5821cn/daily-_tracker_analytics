...
统一数据源管理器 `DataSourceManager` 集成 **6 大数据源**，支持优先级配置和自动故障转移：

| 数据源 | 用途 | 优先级 |
|--------|------|--------|
| **TickFlow** | A股/ETF/港股/美股 实时行情 + 历史 K线 | 1 |
| **Yahoo Finance** | ETF/港股/美股/国际市场数据 | 2 |
| **AkShare** | A股日线/分钟线/资金流向/融资融券 | 3 |
| **东方财富** | ETF/股票 K线数据 | 4 |
| **Tushare** | 专业金融数据（需 token，免费权限有限） | 5 |
| **Baostock** | A股历史数据（稳定备用） | 6 |
...
- 多模型 1日预测
- 趋势判断
- **支持港股成分股**（通过 TickFlow / Yahoo Finance）
...
7. **港股/美股数据支持**: 通过 TickFlow + Yahoo Finance 支持港股、美股 ETF 及成分股分析
